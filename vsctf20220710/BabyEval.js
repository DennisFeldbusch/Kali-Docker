...
function directory(keys) {
  ...
  return (
    "<dl>" +
    keys
      .map((key) => `<dt>${key}</dt><dd><pre>${escape(values[key])}</pre></dd>`)
      .join("") +
    "</dl>"
  );
}
...
app.get("/", (req, res) => {
  ...
  if (payload && typeof payload === "string") {
    const matches =
      /([\.\(\)'"\[\]\{\}<>_$%\\xu^;=]|import|require|process|proto|constructor|app|express|req|res|env|process|fs|child|cat|spawn|fork|exec|file|return|this|toString)/gi.exec(
        payload
      );
    if (matches) {
      res
        .status(400)
        .send(matches.map((i) => `<code>${i}</code>`).join("<br>"));
    } else {
